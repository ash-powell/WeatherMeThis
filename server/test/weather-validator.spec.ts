...
import type { AvgFrequency, Comparison } from '../src/weather/weather.models.js';
...
describe('Average on Matching Days validation', () => {
  it('accepts daily frequency', () => {
    assert.notEqual(validateAnalysisSeries(validAverageMatchingDaysSeries(), 'all'), null);
  });

  it('rejects every nondaily frequency', () => {
    for (const frequency of ['weekly', 'monthly', 'yearly', 'none'] as const) {
      assert.equal(
        validateAnalysisSeries(validAverageMatchingDaysSeries(frequency), 'all'),
        null,
  ...
  it('requires a value comparison', () => {
    assert.equal(
      validateAnalysisSeries(validAverageMatchingDaysSeries('daily', 'none'), 'all'),
      null,
    );
  });
  ...
  it('accepts strict less-than and greater-than comparisons', () => {
    assert.notEqual(
      validateAnalysisSeries(validAverageMatchingDaysSeries('daily', '<'), 'all'),
      null,
    );
    assert.notEqual(
      validateAnalysisSeries(validAverageMatchingDaysSeries('daily', '>'), 'all'),
